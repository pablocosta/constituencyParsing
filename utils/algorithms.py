...
def crf(scores, mask, target=None, mbr=False):
    lens = mask[:, 0].sum(-1)
    total = lens.sum()
    batchSize, seqLen, _ = scores.shape
...
def cky(scores, mask):
    ...
    seqLen, seqLen, batchSize = scores.shape
    s = scores.new_zeros(seqLen, seqLen, batchSize)
    p = scores.new_zeros(seqLen, seqLen, batchSize).long()
